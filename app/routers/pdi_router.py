...
async def upload_batch_pdi(
    ...
    - Battery must already exist (registered via bulk-import).
    ...
    - Single commit after all files for performance.
    """
            ...
            for _, row in df.iterrows():
                ...
                if not battery:
                    summary["errors"].append({
                        "file": file.filename, "id": bid, "reason": "Battery not registered"
                    })
                    continue
                ...
                # Upsert PDI record — always overwrite with latest test data
    ...
    # Single commit covers all files — fast even for 200 files
    db.commit()
